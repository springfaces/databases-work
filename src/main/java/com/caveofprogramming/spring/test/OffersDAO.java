...
public class OffersDAO {
	...
	public int[] create(List<Offer> offers) {
		...
		return jdbc.batchUpdate("insert into offers (name, email , text) "
				+ "values (:id, :name, :email, :text)",  source);
	}
	
	public boolean update(Offer offer) {
		BeanPropertySqlParameterSource params = new BeanPropertySqlParameterSource(offer);
		return jdbc.update("update  offers  set name=:name, text=:text, email=:email where "
				+ "id=:id", params) == 1;
	}

	public boolean create(Offer offer) {
		BeanPropertySqlParameterSource params = new BeanPropertySqlParameterSource(offer);
		return jdbc.update("insert into offers (name, email , text) "
				+ "values (:name, :email, :text)", params) == 1;
	}
